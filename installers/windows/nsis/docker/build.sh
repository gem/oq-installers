...
function fix-scripts {
	for f in $*; do
		sed -i 's/z:\\io\\python-dist\\python3.8\\//g' "$f"
	done
}
...
unzip -q $PY_ZIP -d ../python-dist/python3.8

if [ ! -f $PIP ]; then
    PIP=${HOME}/${PIP}
fi
wine ../python-dist/python3.8/python.exe $PIP
...
for app in oq-engine; do
    ...
    wine ../python-dist/python3.8/python.exe -m pip -q wheel --disable-pip-version-check --no-deps -w ../oq-dist/engine ./${app}
done
...
for app in oq-platform-standalone oq-platform-ipt oq-platform-taxtweb oq-platform-taxonomy; do
    ...
    wine ../python-dist/python3.8/python.exe -m pip -q wheel --disable-pip-version-check --no-deps -w ../oq-dist/tools ./${app}
...
wine ../python-dist/python3.8/python.exe -m pip -q install --disable-pip-version-check --no-warn-script-location --force-reinstall --ignore-installed --upgrade --no-deps --no-index -r oq-engine/requirements-py38-win64.txt -r oq-engine/requirements-extra-py38-win64.txt
...
fix-scripts ${DIR}/python-dist/python3.8/Scripts/*.exe
...
if [[ $OQ_OUTPUT = *"zip"* ]]; then
    cd $DIR/oq-dist
    for d in *; do
		wine ../python-dist/python3.8/python.exe -m pip -q install --disable-pip-version-check --no-warn-script-location --force-reinstall --ignore-installed --upgrade --no-deps --no-index $d/*.whl
    done
    fix-scripts ${DIR}/python-dist/python3.8/Scripts/*.exe
    cp ${DIR}/dist/msvcp/x64/msvcp140.dll ${DIR}/python-dist/python3.8
    echo "Generating ZIP archive"
    ZIP="OpenQuake_Engine_${ini_vers}_${git_time}.zip"
    cd $DIR/python-dist
    zip -qr $DIR/${ZIP} python3.8
    cd $DIR
