...
curl -LO http://sourceware.org/pub/libffi/libffi-3.3.tar.gz
...
LDFLAGS=`pkg-config --libs libffi` ./configure --prefix=$OQ_PREFIX --with-openssl=$OQ_PREFIX --with-ensurepip
make -s -j $NPROC
make -s install
cd ..
