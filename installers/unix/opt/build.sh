...
./configure LDFLAGS=`pkg-config --libs libffi` PKG_CONFIG_PATH="$OQ_PREFIX/lib/pkgconfig/" --prefix=$OQ_PREFIX --with-openssl=$OQ_PREFIX --without-system-ffi --with-ensurepip
make -s -j $NPROC
make -s install
cd ..
